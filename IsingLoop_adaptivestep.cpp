...
long int isteps = msteps/cool_ratio;
...
double magpow2Bins[num_bins] = {0};

double magpow2Values[num_temp_step] = {0};

double magpow2Errors[num_temp_step] = {0};
...
void results(ofstream& myfile3, ofstream& myfile4, ofstream& myfile6, ofstream& myfile7, int tempstepnum);
...
int main() {
    ...
    writelog << "Now processing T = " << temp << ", beta = " << beta << "\n\n";
      ...
      // now check for proper convergence

      // compute mean and standard deviation

     	double meanMagpow2 = 0;

			for (int q=0; q<num_bins; q++) {

				meanMagpow2 = meanMagpow2 + abs( magpow2Bins[q] );

			}

      meanMagpow2 = meanMagpow2 / ( (double) num_bins );

      double standardDeviation = 0;

      for (int p=0; p<num_bins; p++) {

				standardDeviation = standardDeviation + ( abs( magpow2Bins[p] ) - meanMagpow2 )*( abs( magpow2Bins[p] ) - meanMagpow2 );

      }

      standardDeviation = standardDeviation / ( (double) num_bins );

      standardDeviation = sqrt(standardDeviation);

      //double magError = standardDeviation / ( sqrt( ( (double) num_bins ) ) );

      double magpow2Error = standardDeviation;

      double percError = magpow2Error / meanMagpow2; // meanMag will be positive

      if (percError < min_error ) {

				// number of steps is larger than needed

				convergence = true;

				magpow2Values[y] = meanMagpow2;

				magpow2Errors[y] = magpow2Error;

				long int tent_num_steps = msteps/update_ratio;

				if (tent_num_steps < min_msteps) {

					cout << "The minimum number of steps has been reached...\n" << endl;

					continue;

				} else {

					msteps = tent_num_steps;

					isteps = msteps/cool_ratio;

					cout << "The number of measuring steps has been decreased to " << msteps << "\n" << endl;

					continue;

				}

      } else if (percError > max_error ) {

				long int tent_num_steps = msteps*update_ratio;

				if (tent_num_steps > max_msteps) {

					cout << "The maximum number of steps has been exceeded without reaching convergence...\n" << endl;
					...
					// calculation will be repeated with more steps

					msteps = tent_num_steps;

					isteps = msteps / cool_ratio;

					cout << "The number of measuring steps has been increased to " << msteps << ", calculation will repeat...\n" << endl;
    ...
    cout << "The average magnetization is " << magpow2Values[y] << " +/- " << magpow2Errors[y] << endl;

    cout << "\n\n" << endl;

		results(writemag, writeenr, writesu, writemag_2vstemp, y);
...
void results(ofstream& myfile3, ofstream& myfile4, ofstream& myfile6, ofstream& myfile7, int tempstepnum) {
	...
	if (temp == final_temp){
		...
		myfile7 << magpow2Values[tempstepnum];
